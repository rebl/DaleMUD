...
if (ch->specials.fighting && ch->specials.fighting != ch)       {
   ...
   if (!IS_AFFECTED(ch, AFF_PROTECT_FROM_EVIL) && number(0,6)==0) {
      act("$n prayed to $s diety to protect $m from evil.", 1, ch,0,0,TO_ROOM);
...
int ChurchBell(struct char_data *ch, int cmd, char *arg, struct room_data *rp, int type)
{
	if (cmd==PULL)
 {
  ...
  if(!strcmp("rope",buf)) {
     send_to_all("The bells of Shadowdale's church sound 'GONG! GONG! GONG! GONG!'\n");
	return(TRUE);
    } /* end strcmpi() */
   return(FALSE);    
 } /* end pull */
...
if (type == EVENT_DEATH && ch->in_room == SLAV_LIVE_ROOM) {
  struct char_data *v;
	...
	send_to_all("The chruch bells of Shadowdale sound off in a chorus of happiness!\n");	
...
if (cmd || !AWAKE(mob))
 ...
 if (mob->specials.fighting) {
  ...
  if (action>=75) {
     act("$n turns one of $m eye stalks at $N!",FALSE,mob,0,mob->specials.fighting,TO_NOTVICT);
     act("$n turns a evil looking eye stalk at you!",FALSE,mob,0,mob->specials.fighting,TO_VICT);
     cast_disintergrate(GetMaxLevel(mob),mob,"",SPELL_TYPE_SPELL,mob->specials.fighting,0);
	return(TRUE);
    } else
  if (action>=50) {
     act("$n turns one of $m eye stalks at $N!",FALSE,mob,0,mob->specials.fighting,TO_NOTVICT);
