...
void char_from_room(struct char_data *ch)
{
    ...
    if (rp == NULL) {
        sprintf(buf,
                "ERROR: char_from_room: %s was not in a valid room (%ld)",
                (!IS_NPC(ch) ? (ch)->player.name : (ch)->player.
                 short_descr), ch->in_room);
        klog(buf);
        return;
    }
    ...
    if (ch == rp->people)       /* head of list */
        ...
        if (i)
            ...
            sprintf(buf,
                    "Opps! %s was not in people list of his room %ld!",
                    (!IS_NPC(ch) ? (ch)->player.name : (ch)->player.
                     short_descr), ch->in_room);
            klog(buf);
        }
        ...
        if (ch->equipment[WEAR_LIGHT]->obj_flags.type_flag == ITEM_LIGHT) {
            if (rp->sector_type != SECT_UNDERWATER) {
                ...
                if (ch->equipment[WEAR_LIGHT]->obj_flags.value[2] > 0) {
                    send_to_char
                        ("Your light source is extinguished instantyl!\n\r",
                         ch);
...
void obj_to_char(struct obj_data *object, struct char_data *ch)
{
    ...
    if (!ch) {
        sprintf(buf,
                "!ch in obj_to_char for object <%s>,could be bogus maximum for the obj in the zone file",
                object->name);
        slog(buf);
        klog(buf);
        return;
...
void obj_from_obj(struct obj_data *obj)
{
    ...
    assert(!obj->carried_by && !obj->equipped_by
           && obj->in_room == NOWHERE);
