...
if (IS_PC(ch) || IS_SET(ch->specials.act,ACT_POLYSELF))    
  switch(loc)
    ...
    case APPLY_STR:
      ...
      if (GET_STR(ch) > 18 && GET_STR(ch) > MaxStrForRace(ch)) {
         GET_ADD(ch) += ((GET_STR(ch)-18)*10);
         if (GET_ADD(ch) < 0) GET_ADD(ch) = 0;
        }
        if (GET_ADD(ch) > 100 && (GET_STR(ch)+1 <= MaxStrForRace(ch))) {
         GET_STR(ch) +=1;
         GET_ADD(ch) = 0;
        }
        if (GET_ADD(ch) > 100) 
 ...
 /* negatives make saving throws better */
   ...
   if ((int)(ch->skills[SKILL_FIND_TRAP].learned+ mod) >=100)
      ch->skills[SKILL_FIND_TRAP].learned=100; else
   ...
   if ((int)(ch->skills[SKILL_RIDE].learned+ mod) >=100)
      ch->skills[SKILL_RIDE].learned=100; else
...
/* set these attribs to the mod givin the attrib setting the VALUE, not mod */
	case APPLY_T_STR:
	case APPLY_T_INT:
	case APPLY_T_DEX:
	case APPLY_T_WIS:
...
#if LOG_DEBUG
      klog("Unknown apply adjust attempt (handler.c, affect_modify).");
      klog(ch->player.name);
#endif      
      break;
...
void affect_remove( struct char_data *ch, struct affected_type *af )
{
  ...
  if (!ch->affected) {
    klog("affect removed from char without affect");
    klog(GET_NAME(ch));
    return;
  }
  ...
  if (!af) {
    klog("WARNING!!!! af is NULL! in affect_remove()");
    return;
   }
...
for(hjp = ch->affected; (hjp) && (hjp->next) && (hjp->next != af);hjp = hjp->next);    
    if (hjp->next != af) {
      klog("Could not locate affected_type in ch->affected. (handler.c, affect_remove)");
      return;
    }
...
#if 0
	hjp=ch->affected;
	while((hjp) && hjp->type != skill) /* flip through the list searching */
	 hjp=hjp->next;

	if (hjp->type == skill)	/* if we found remove it */
...
void char_from_room(struct char_data *ch)
{
  ...
  if (ch->in_room == NOWHERE) {
    klog("NOWHERE extracting char from room (handler.c, char_from_room)");
    return;
  }
...
char buf[255];
 ...
 if (!object) {
  klog("!object in obj_to_char!");
	return;
}
 if (!ch) {
sprintf(buf,"!ch in obj_to_char for object <%s>,could be bogus maximum for the obj in the zone file",object->name);
  slog(buf);
  klog(buf);
	return;
}
 ...
 if (!object->in_obj && !object->carried_by && !object->equipped_by &&
	 ...
	 char buffer[1024];
	 sprintf(buffer,"!object->in_obj && !object->carried_by && !object->equipped_by &&	 object->in_room == NOWHERE");
	klog(buffer);	 
	sprintf(buffer,"Obj name (%s) Obj Vnum (%ld)",object->name,ObjVnum(object));
	klog(buffer);
	return;
    }
...
void obj_from_char(struct obj_data *object)
{
  ...
  if (!object) {
    klog("No object to be take from char.");
    assert(0);
  }
  
  
  if (!object->carried_by) {
    klog("this object is not carried by anyone");
/*    assert(0); */
  }
  
  if (!object->carried_by->carrying) {
    klog("No one is carrying this object");
    assert(0);
  }

  if (object->in_obj) {
    klog("Obj in more than one place.");
    assert(0);
  }
  ...
  if (object->carried_by->carrying == object)   /* head of list */
  ...
  else
    {
      ...
      if (!tmp) {
	klog("Couldn't find object on character");
	assert(0);
      }
...
if (pos<0 || pos>MAX_WEAR) {
  klog("wear pos > MAX_WEAR or < 0 in handler.c");
  return;
 }
...
/*  assert(pos>=0 && pos<MAX_WEAR); */
  ...
  if (obj->carried_by) {
    klog("EQUIP: Obj is carried_by when equip.");
    assert(0);
  }
  
  if (obj->in_room!=NOWHERE) {
    klog("EQUIP: Obj is in_room when equip.");
    assert(0);
    return;
  }
...
if ( obj_index[obj->item_number].func == EvilBlade ||
  ...
  if (!CheckEgo(ch,obj))  {
   if (ch->in_room != NOWHERE) {
      ...
      do_save(ch,"",0);
        ...
        klog("Ch->in_room = NOWHERE on anti-ego item!");
       return; 	
     } 
  } 			/* end item ego checks */
  ...
  if (!CheckGetBarbarianOK(ch,obj))   {
   if (ch->in_room != NOWHERE) {
      ...
      return;
        ...
        klog("Ch->in_room = NOWHERE on anti-barb item!");
      return;
    }
  ...
  if (ItemAlignClash(ch, obj) && (GetMaxLevel(ch) < IMPLEMENTOR)) 
  {
    ...
    {
      klog("ch->in_room = NOWHERE when equipping char.");
      assert(0);
    }
  }
}  
...
klog ("pos > MAX_WEAR || pos < 0 in handler.c unequip");
return;
}
...
void obj_from_room(struct obj_data *object)
{
  ...
  if (object->in_room <= NOWHERE) {
    if (object->carried_by || object->equipped_by) {
       klog("Eek.. an object was just taken from a char, instead of a room");
       assert(0);
    }
  ...
  if (object == real_roomp(object->in_room)->contents)  /* head of list */
	...
	klog("Couldn't find object in room");
	assert(0);
      }
    }
...
void extract_obj(struct obj_data *obj)
{
  ...
  if (obj->equipped_by) 
  {
    ...
    {
      klog("Extract on equipped item in slot -1 on:");
      klog(obj->equipped_by->player.name);
      klog(obj->name);
      return;
    }
  } else 
  ...
  if (object_list == obj )       /* head of list */
    ...
    if(temp1) {
      ...
      klog("Couldn't find object in object list.");
      assert(0);
    }
  }
...
{
check_affected("before extract char");
  extract_char_smarter(ch, NOWHERE);
check_affected("after extract char");
}
...
*/
  ...
  if (ch->in_room == NOWHERE) {
    klog("NOWHERE extracting char. (handler.c, extract_char)");
    /*
  ...
  if (ch == character_list)  
    ...
    if(k)
      ...
      klog("Trying to remove ?? from character_list.(handler.c,extract_char)");
      exit(0);
    }
  }
...
/*  char *strdup(char *str); */
  ...
  if(amount<=0) {
      klog("ERROR: Try to create negative money.");
      assert(0);
    }
...
struct obj_data *create_money( int amount )
{
    ...
    if ( amount <= 0 )
    {
	klog( "Create_money: zero or negative money. handler.c");
	amount = 1;
    }
