...
#define REBOOT_AT    3  /* 0-23, time of optional reboot if -e lib/reboot */
...
void show_string(struct descriptor_data *d, char *input)
{
  ...
  for (scan = buffer;; scan++, d->showstr_point++) {
    if((((*scan = *d->showstr_point) == '\n') || (*scan == '\r')) &&
       ((toggle = -toggle) < 0)) {
      lines++;
      if(strlen(buffer)>MAX_STRING_LENGTH-265) i = lines;
    }
    else if (!*scan || (lines >= i))      {
      *scan = '\0';
     SEND_TO_Q(ParseAnsiColors(IS_SET(d->character->player.user_flags,USE_ANSI),buffer),d);
